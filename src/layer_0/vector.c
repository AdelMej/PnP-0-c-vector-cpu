...
Vector* vector_new(size_t initial_capacity) {
    ...
    if (vector_new == NULL) return (NULL);
...
int vector_set(Vector* v, size_t idx, vector_data_t d) {
    ...
    if (idx >= v->size) return VECTOR_ERR_OOB;
...
int vector_pop(Vector* v, vector_data_t* out) {
    ...
    if (v->size == 0) return VECTOR_ERR_EMPTY;
...
int vector_resize(Vector* v, size_t resize) {
    ...
    if (resize == 0) return VECTOR_ERR_OOB;

    temp = realloc(v->data, sizeof(vector_data_t) * resize);
    if (temp == NULL) return VECTOR_ERR_ALLOC;
...
int vector_size(Vector* v, size_t* out) {
    if (v == NULL || out == NULL) return VECTOR_ERR_NULL;
...
int vector_insert(Vector* v, size_t idx, vector_data_t value) {
    ...
    if (v->size + 1 > v->capacity) {
        temp = realloc(v->data, sizeof(vector_data_t) * v->capacity *
                                    VECTOR_GROWTH_FACTOR);
        if (temp == NULL) return VECTOR_ERR_ALLOC;
        v->data = temp;
        v->capacity *= VECTOR_GROWTH_FACTOR;
    ...
    for (i = v->size; i > idx; i--) v->data[i] = v->data[i - 1];
...
int vector_foreach(Vector* v, void (*func)(vector_data_t* elem)) {
    ...
    if (v == NULL || func == NULL) return VECTOR_ERR_NULL;

    for (i = 0; i < v->size; i++) func(&v->data[i]);
...
Vector* vector_map(Vector* v, vector_data_t (*func)(vector_data_t elem)) {
    ...
    if (v == NULL || func == NULL) return NULL;

    copy = vector_new(v->capacity);
    if (copy == NULL) return NULL;

    for (i = 0; i < v->size; i++) copy->data[i] = func(v->data[i]);
...
ssize_t vector_find(Vector* v, int (*func)(vector_data_t elem)) {
    ...
    if (v == NULL || func == NULL) return -1;

    for (i = 0; i < v->size; i++) {
        if (func(v->data[i])) return (ssize_t)i;
    }
...
int vector_dot(Vector* a, Vector* b, vector_data_t* out) {
    ...
    if (a == NULL || b == NULL || out == NULL) return VECTOR_ERR_NULL;

    if (a->size != b->size) return VECTOR_ERR_SIZE;

    for (i = 0; i < a->size; i++) result += a->data[i] * b->data[i];
...
int vector_norm(Vector* v, vector_data_t* out) {
    ...
    for (i = 0; i < v->size; i++) result += v->data[i] * v->data[i];
...
Vector* vector_add(Vector* a, Vector* b) {
    ...
    for (i = 0; i < a->size; i++) result->data[i] = a->data[i] + b->data[i];
